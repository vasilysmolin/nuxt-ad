...
export default {
  ...
  layout: 'office',
  async mounted() {
    await this.$store.dispatch('realty/getItem', {id: this.$route.params.id}).then(() => {
      this.checkState();
    });
  ...
  computed: {
    realty() {
      return _.cloneDeep(this.$store.getters['realty/realty']);
    },
    states: {
      get() {
  ...
  methods: {
    submitted() {
      this.$axios.$put(`realties/${this.realty.id}/state`, {state: this.realty.state});
      this.$axios.$put(`realties/${this.realty.id}`, {reason: this.reason});
    },
    format(date) {
  ...
  head() {
    return {
      title: `${this.realty.title} | Вакансии без ограничений на Tapigo.ru | Работа`,
      meta: [
