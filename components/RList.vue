<template>
  <section class="container flex flex-col items-center mt-[10px] pb-10">
    <section class="flex flex-col w-[95%] sm:max-w-screen-sm">
      ...
      <button @click="addItems({skip: resumes.length})" type="button" class="w-full inline-block mt-6 px-6 py-2 border-2 border-blue-600 text-blue-600 font-bold text-normal leading-normal rounded hover:border-black hover:text-black focus:outline-none focus:ring-0 transition duration-150 ease-in-out">Смотреть дальше</button>
    </section>
  </section>
</template>
