<template>
  <section class="mt-7 flex flex-col justify-start items-start w-full">
    <article @click="vacancies" class="p-3 flex flex-col rounded-lg w-full bg-[#FFF2CB]">
      <IconListServicesJobsVacancies/>
      <div>
        <h2 class="mt-4 text-sm font-black">Вакансии</h2>
        <h3 class="mt-1 text-[11px] leading-snug">Создавайте неограниченное количество бесплатных вакансий для своей компании. Делегируйте контроль или размещение своему представителю.</h3>
      </div>
    </article>
    <article @click="resume" class="mt-4 p-3 flex flex-col rounded-lg w-full bg-[#FFECD1]">
      <IconListServicesJobsResume/>
      <div>
        <h2 class="mt-4 text-sm font-black">Резюме</h2>
        <h3 class="mt-1 text-[11px] leading-snug">Размещайте без ограничений бесплатные резюме для поиска работы. Получайте приглашения на собеседования сразу на Тапиго.</h3>
      </div>
    </article>
  </section>
</template>
...
import IconListServicesJobsVacancies from "./icons/IconListServicesJobsVacancies"
import IconListServicesJobsResume from "./icons/IconListServicesJobsResume"
