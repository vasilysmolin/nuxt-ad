...
export default {
  ...
  methods: {
    ...
    getParamsSelect(ad) {
      return _.sortBy(_.filter(ad?.parameters, (item) => this.isSelect(item.filter)), (item) => item.filter.sort);
    },
    getParamsRange(ad) {
      return _.sortBy(_.filter(ad?.parameters, (item) => this.isRange(item.filter)), (item) => item.filter.sort);
    },
    isFilter(cat) {
