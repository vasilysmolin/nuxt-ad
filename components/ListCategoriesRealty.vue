<template>
  ...
  <article v-bind:class="`break-inside article-card-catalog-index bg-[${getColor(category)}]`">
    <h2>
      <nuxt-link :to="categoryLink(category)" :class="`border-b-[1px] border-gray-400 ${getColorH1(category)}`">
        {{ category.name }}
      </nuxt-link>
    </h2>
    ...
    <ul v-for="cat in category.categories " class="list-disc list-inside leading-7">
      <li>
        <nuxt-link :to="categoryLink(cat)" :class="`${getColorLi(category)}`">{{ cat.name }}</nuxt-link>
      </li>
    </ul>
  </article>
